...
d = sigma .* eta ./ sqrt(n .* log(log(n))); % half length of confidence interval 
...
while max_ub - ( max_x - d(max_i) ) > delta 
    ...
    for b1 = (B-b):-b:0
        n_tmp(max_i) = n_tmp(max_i) - b;
        d_tmp(max_i) = sigma(max_i) * eta ./ sqrt(n_tmp(max_i) .* log(log(n_tmp(max_i))));
        n_tmp(max_ub_i) = n_tmp(max_ub_i) + b;
        d_tmp(max_ub_i) = sigma(max_ub_i) * eta ./ sqrt(n_tmp(max_ub_i) .*log(log(n_tmp(max_ub_i) )));
