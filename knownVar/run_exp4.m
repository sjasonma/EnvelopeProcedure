...
for config_id = [2,3]
    for iter = 1:N_iter
        ...
        EP1_known;
        c_NP(iter, config_id) = mu(max_i) > max(mu) - delta;
        n_NP(iter, config_id) = sum(n);
...
for config_id = 1:12
    config_id
    fprintf('%.3f, %.3f, %.3f\n', mean(c_KN(:, config_id)), mean(c_BIZ(:, config_id)), mean(c_NP(:, config_id)))
    fprintf('%e, %e, %e\n', mean(n_KN(:, config_id)), mean(n_BIZ(:, config_id)), mean(n_NP(:, config_id)))
    fprintf('%e, %e, %e\n', 1.96*std(n_KN(:, config_id))/sqrt(N_iter), 1.96*std(n_BIZ(:, config_id))/sqrt(N_iter), 1.96*std(n_NP(:, config_id))/sqrt(N_iter))
    ...
    fprintf('%f, %f\n', mean(n_NP(:, config_id) ./ n_KN(:, config_id)), mean(n_NP(:, config_id) ./ n_BIZ(:, config_id)))
    fprintf('%f, %f\n', std(n_NP(:, config_id) ./ n_KN(:, config_id))*1.96/sqrt(N_iter), std(n_NP(:, config_id) ./ n_BIZ(:, config_id))*1.96/sqrt(N_iter))
    fprintf('\n')
end
